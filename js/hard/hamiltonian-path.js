/**
 * Challenge
 * Using the JavaScript language, have the function HamiltonianPath(strArr)
 * take strArr which will be an array of length three. The first part of the
 * array will be a list of vertices in a graph in the form (A,B,C,...), the
 * second part of the array will be the edges connecting the vertices in the
 * form (A-B,C-D,...) where each edge is bidirectional. The last part of the
 * array will be a set of vertices in the form (X,Y,Z,...) and your program
 * will have to determine whether or not the set of vertices given forms a
 * Hamiltonian path on the graph which means that every vertex in the graph is
 * visited only once in the order given.
 *
