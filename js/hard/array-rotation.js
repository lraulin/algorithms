 ...
 * array as a string, so for this example your program would return 4161023.
 * The first element in the array will always be an integer greater than or
...
function ArrayRotation(arr) {
  const newArr = arr.slice(arr[0]).concat(arr.slice(0, arr[0]));
  return newArr.join('');
}

// const test = [3, 2, 1, 6];
const test = [4, 3, 4, 3, 1, 2];
console.log(ArrayRotation(test));
